...
export class ClaudeRunner implements IAiRunner {
  ...
  async run(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const pendingToolUses = new Map<string, { filePath: string; generation: number }>();
  ...
  private handleLine(
    line: string,
    pendingToolUses: Map<string, { filePath: string; generation: number }>,
    workingDir: string,
    onProgress?: ProgressCallback
  ): void {
    ...
    if (event.type === 'assistant') {
      ...
      for (const item of assistantEvent.message.content) {
        ...
        if (!this.diffManager.monitoringEnabled) { continue; }
        pendingToolUses.set(toolUse.id, { filePath, generation: this.diffManager.monitoringGeneration });
    ...
    if (event.type === 'tool') {
      ...
      const pending = pendingToolUses.get(toolEvent.tool_use_id);
      if (pending) {
        pendingToolUses.delete(toolEvent.tool_use_id);
        if (pending.generation !== this.diffManager.monitoringGeneration) { return; }
