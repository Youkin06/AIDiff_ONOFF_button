  ...
  private async navigate(direction: number): Promise<void> {
    ...
    if (pendingFiles.length === 1) {
      if (normalizePath(pendingFiles[0]) !== currentPath) {
        // User hit nextFile/prevFile — manual intent, always open the diff editor.
        await this.diffManager.openDiff(pendingFiles[0], true);
      }
      return;
    }
    ...
    await this.diffManager.openDiff(targetPath, true);
  }
