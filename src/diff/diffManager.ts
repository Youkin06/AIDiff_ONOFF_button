...
export class DiffManager {
  ...
  public monitoringGeneration = 0;

  get monitoringEnabled(): boolean { return this._monitoringEnabled; }

  set monitoringEnabled(enabled: boolean) {
    if (enabled !== this._monitoringEnabled) {
  ...
  async openDiff(filePath: string, force = false): Promise<void> {
    if (!this.monitoringEnabled && !force) { return; }
    const absPath = normalizePath(filePath);
