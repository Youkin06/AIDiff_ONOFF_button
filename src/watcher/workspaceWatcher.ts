...
import { FileSnapshotStore, isTextFile, canSnapshotFile, MAX_MONITORED_FILE_BYTES } from './fileSnapshotStore';
...
export class WorkspaceWatcher {
  ...
  private watchVscodeEvents(): void {
    const d = vscode.workspace.onDidSaveTextDocument((doc) => {
      ...
      if (doc.uri.scheme !== 'file' || isExcludedPathSegment(filePath) || !canSnapshotFile(filePath)) { return; }
  ...
  private handleExternalWrite(filePath: string): void {
    if (!this.diffManager.monitoringEnabled) { return; }
    ...
    setTimeout(() => {
      if (generation !== this.generation || !this.diffManager.monitoringEnabled) { return; }
      ...
      try {
        if (!canSnapshotFile(absPath)) { return; }
  ...
  updateSnapshot(filePath: string, content: string): void {
    if (isExcludedPathSegment(filePath) || Buffer.byteLength(content, 'utf8') > MAX_MONITORED_FILE_BYTES) { return; }
