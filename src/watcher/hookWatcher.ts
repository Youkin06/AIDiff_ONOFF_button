...
export class HookWatcher {
  ...
  // Keep consuming our workspace's signals while paused so they cannot replay later.
  monitoringChanged(): void {
    this.generation++;
  ...
  start(): void {
    ...
    this.watcher = fs.watch(this.signalDir, (_event, filename) => {
      if (filename && filename.endsWith('.json')) {
        ...
        setTimeout(() => {
          if (this.watcher && generation === this.generation) { this.processSignal(signalPath); }
        }, 150);
      }
    });
  ...
  private processSignal(signalPath: string): void {
    try {
      ...
      if (!this.diffManager.monitoringEnabled || signal.timestamp <= this.resumedAt) { return; }
      ...
      if (snapshotMetaPath) {
        try {
          ...
          if (typeof meta.timestamp === 'number' && meta.timestamp <= this.resumedAt) { return; }
